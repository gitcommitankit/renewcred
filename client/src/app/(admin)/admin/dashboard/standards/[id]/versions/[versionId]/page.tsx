...
export default function VersionEditorPage() {
  ...
  const splitEditorContent = useCallback(
    (doc: TiptapDocument): { title: string; content: TiptapDocument } => {
  ...
  const doSave = useCallback(
    async (doc: TiptapDocument, sectionId: string) => {
  ...
  const handleCreateSection = async () => {
    ...
    try {
      ...
      if (!selectedParentId) {
        ...
        const parentBase = parent?.number.endsWith('.0')
          ? parent.number.slice(0, -2)
          : parent?.number;
      ...
      toast.error(
        (err as { data?: { message?: string } })?.data?.message || 'Failed to create section'
      );
    }
  };
        ...
        <div className="w-full md:w-64 flex flex-col bg-white border-b md:border-b-0 md:border-r border-warm-gray-200 overflow-y-auto shrink-0 md:max-h-full max-h-40vh">
              ...
              >
                ...
                {allSections.map((s) => (
                  <option key={s.id} value={s.id}>
                    {'  '.repeat(
                      s.parentId
                        ? allSections.find((p) => p.id === s.parentId)?.parentId
                          ? 2
                          : 1
                        : 0
                    )}
          ...
          <div className="flex-1 py-2 px-1">
            {allSections.length === 0 ? (
              <p className="text-xs text-warm-gray-500 text-center py-6 px-3">
          ...
          {!activeSection ? (
            ...
            <div className="mx-auto">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-xs font-mono text-warm-gray-500 bg-warm-gray-200 px-2 py-1 rounded shrink-0">
                  {activeSection.number}
                </span>
                <h2 className="flex-1 text-lg font-bold text-charcoal-900 px-1 py-0.5 select-none">
                  {activeSection.title}
                </h2>
              </div>
              <TiptapEditor
